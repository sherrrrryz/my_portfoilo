...
/* Click-to-copy for the CTA. Not a mailto: that only does something when the
   ...
   denied, so there is never a dead end. Its own file because the Touch Hot
   Zone page is a server component; same split as ThemeToggle. Duplicated per
...
export default function CopyEmail({ label }: { label: string }) {
  ...
  return (
    <>
      ...
      <div
        className={`thz-toast${toast ? " thz-toast--on" : ""}`}
        role="status"
        aria-live="polite"
      >
        {toast}
