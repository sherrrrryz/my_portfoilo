...
| AppLovin OOBE case study | built | `/projects/applovin-oobe` | Short visual-first landing (`oobe.css`, `alo-` prefix, consumes `tokens.css`). Stat bands + SVG diagrams + sanitized flow images from `public/oobe/`. Not linked from the homepage yet. |
